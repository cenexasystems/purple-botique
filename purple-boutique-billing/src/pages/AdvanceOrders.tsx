...
export default function AdvanceOrders({ onOrderCompleted }: AdvanceOrdersProps = {}) {
  ...
  const receivePayment = async (event: FormEvent) => {
    ...
    try {
      ...
      const finalAmount = Math.max(0, paymentOrder.remaining_balance - couponDiscount - manualDiscountNum)
      ...
      if (manualDiscountNum > 0) parts.push(`Manual Discount: -RM${manualDiscountNum.toFixed(2)}`)
      ...
      setOrders(rows => rows.map(row => row.id === completed.id ? completed : row)); onOrderCompleted?.(completed); setPaymentOrder(null); setPaymentForm({ method: 'cash', remarks: '' }); setAppliedCoupon(null); setCouponInput(''); setCouponError(''); setManualDiscount(''); setNotice(`${result.invoice_no} generated once. The full ${formatCurrency(completed.total_amount)} is now recognized as revenue.`)
  ...
  return <div className="space-y-5">
    ...
    {paymentOrder && <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/55 p-4"><form onSubmit={receivePayment} className="w-full max-w-md max-h-[92vh] overflow-y-auto rounded-3xl bg-white p-6 shadow-2xl"><div className="mb-4 flex justify-between items-start"><div><p className="text-xs font-black text-violet-600">{paymentOrder.deposit_id}</p><h3 className="text-xl font-black">Receive Remaining Payment</h3></div><button type="button" onClick={()=>setPaymentOrder(null)} className="shrink-0 ml-2"><X/></button></div><div className="mb-5 rounded-2xl bg-emerald-50 p-5 text-center">{(() => { const couponDisc = appliedCoupon ? Math.round(paymentOrder.remaining_balance * (appliedCoupon.percentage / 100) * 100) / 100 : 0; const manualDisc = Math.max(0, Number(manualDiscount) || 0); const finalAmt = Math.max(0, paymentOrder.remaining_balance - couponDisc - manualDisc); return (<><p className="text-xs font-black uppercase text-emerald-700">Remaining Amount</p><p className={`mt-1 font-black text-emerald-800 ${(couponDisc > 0 || manualDisc > 0) ? 'text-xl line-through opacity-60' : 'text-3xl'}`}>{formatCurrency(paymentOrder.remaining_balance)}</p>{(couponDisc > 0 || manualDisc > 0) && (<><div className="mt-2 space-y-0.5 text-xs text-emerald-700">{couponDisc > 0 && <p>Coupon {appliedCoupon!.code} ({appliedCoupon!.percentage}%): -{formatCurrency(couponDisc)}</p>}{manualDisc > 0 && <p>Manual Discount: -{formatCurrency(manualDisc)}</p>}</div><p className="mt-2 text-2xl font-black text-emerald-900">You Pay: {formatCurrency(finalAmt)}</p></>)}</>)})()}</div><div className="space-y-4"><Field label="Coupon Code (Optional)"><div className="flex gap-2">{appliedCoupon ? (<div className="flex flex-1 items-center justify-between rounded-xl bg-violet-50 px-3 py-2.5 text-sm font-black text-violet-700"><span>{appliedCoupon.code} — {appliedCoupon.percentage}% OFF</span><button type="button" onClick={removeCoupon} className="ml-2 text-red-500 hover:text-red-700"><X size={14}/></button></div>) : (<><input list="adv-coupon-list" className={`${inputClass} flex-1`} value={couponInput} onChange={e=>{setCouponInput(e.target.value.toUpperCase());setCouponError('')}} placeholder="Enter coupon code" /><datalist id="adv-coupon-list">{availableCoupons.map(c=><option key={c.code} value={c.code}/>)}</datalist><button type="button" onClick={applyCoupon} className="rounded-xl bg-violet-600 px-4 py-2.5 text-sm font-black text-white hover:bg-violet-700">Apply</button></>)}</div>{couponError && <p className="mt-1 text-xs font-semibold text-red-600">{couponError}</p>}</Field><Field label="Manual Discount (RM)"><input type="number" min="0" step="0.01" className={inputClass} value={manualDiscount} onChange={e=>setManualDiscount(e.target.value)} placeholder="0.00" /></Field><Field label="Payment Method"><select className={inputClass} value={paymentForm.method} onChange={e=>setPaymentForm({...paymentForm,method:e.target.value as AdvancePaymentMethod})}><option value="cash">Cash</option><option value="upi">QR</option><option value="card">Card</option></select></Field><Field label="Remarks"><textarea className={inputClass} value={paymentForm.remarks} onChange={e=>setPaymentForm({...paymentForm,remarks:e.target.value})}/></Field></div><p className="mt-4 rounded-xl bg-amber-50 p-3 text-xs font-semibold text-amber-800">Confirmation marks the order Completed, creates one official invoice, and recognizes the full {formatCurrency(paymentOrder.total_amount)} as revenue.</p><button disabled={saving} className="mt-5 w-full rounded-xl bg-emerald-600 py-3 font-black text-white disabled:opacity-50">{saving?'Processing...':'Confirm Final Payment'}</button></form></div>}
