...
export default function AdvanceOrders({ onOrderCompleted }: AdvanceOrdersProps = {}) {
  ...
  const receivePayment = async (event: FormEvent) => {
    ...
    try {
      ...
      const finalAmount = Math.max(0, paymentOrder.remaining_balance - couponDiscount)
      const remarksWithCoupon = appliedCoupon
        ? `${paymentForm.remarks ? paymentForm.remarks + ' | ' : ''}Coupon: ${appliedCoupon.code} (-${appliedCoupon.percentage}%) = -RM${couponDiscount.toFixed(2)}`.trim()
        : paymentForm.remarks
      ...
      setOrders(rows => rows.map(row => row.id === completed.id ? completed : row)); onOrderCompleted?.(completed); setPaymentOrder(null); setPaymentForm({ method: 'cash', remarks: '' }); setAppliedCoupon(null); setCouponInput(''); setCouponError(''); setNotice(`${result.invoice_no} generated once. The full ${formatCurrency(completed.total_amount)} is now recognized as revenue.`)
  ...
  return <div className="space-y-5">
    ...
    {paymentOrder && <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/55 p-4"><form onSubmit={receivePayment} className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl"><div className="mb-4 flex justify-between"><div><p className="text-xs font-black text-violet-600">{paymentOrder.deposit_id}</p><h3 className="text-xl font-black">Receive Remaining Payment</h3></div><button type="button" onClick={()=>setPaymentOrder(null)}><X/></button></div><div className="mb-5 rounded-2xl bg-emerald-50 p-5 text-center"><p className="text-xs font-black uppercase text-emerald-700">Remaining Amount</p><p className="mt-1 text-3xl font-black text-emerald-800">{formatCurrency(paymentOrder.remaining_balance)}</p>{appliedCoupon && (<><p className="mt-2 text-xs font-black text-violet-700">Coupon {appliedCoupon.code}: -{appliedCoupon.percentage}%</p><p className="mt-1 text-2xl font-black text-emerald-900">You Pay: {formatCurrency(Math.max(0, paymentOrder.remaining_balance - Math.round(paymentOrder.remaining_balance * (appliedCoupon.percentage / 100) * 100) / 100))}</p></>)}</div><div className="space-y-4"><Field label="Coupon Code (Optional)"><div className="flex gap-2">{appliedCoupon ? (<div className="flex flex-1 items-center justify-between rounded-xl bg-violet-50 px-3 py-2.5 text-sm font-black text-violet-700"><span>{appliedCoupon.code} — {appliedCoupon.percentage}% OFF</span><button type="button" onClick={removeCoupon} className="ml-2 text-red-500 hover:text-red-700"><X size={14}/></button></div>) : (<><input className={`${inputClass} flex-1`} value={couponInput} onChange={e=>{setCouponInput(e.target.value.toUpperCase());setCouponError('')}} placeholder="Enter coupon code" /><button type="button" onClick={applyCoupon} className="rounded-xl bg-violet-600 px-4 py-2.5 text-sm font-black text-white hover:bg-violet-700">Apply</button></>)}</div>{couponError && <p className="mt-1 text-xs font-semibold text-red-600">{couponError}</p>}</Field><Field label="Payment Method"><select className={inputClass} value={paymentForm.method} onChange={e=>setPaymentForm({...paymentForm,method:e.target.value as AdvancePaymentMethod})}><option value="cash">Cash</option><option value="upi">QR</option><option value="card">Card</option></select></Field><Field label="Remarks"><textarea className={inputClass} value={paymentForm.remarks} onChange={e=>setPaymentForm({...paymentForm,remarks:e.target.value})}/></Field></div><p className="mt-4 rounded-xl bg-amber-50 p-3 text-xs font-semibold text-amber-800">Confirmation marks the order Completed, creates one official invoice, and recognizes the full {formatCurrency(paymentOrder.total_amount)} as revenue.</p><button disabled={saving} className="mt-5 w-full rounded-xl bg-emerald-600 py-3 font-black text-white disabled:opacity-50">{saving?'Processing...':'Confirm Final Payment'}</button></form></div>}
