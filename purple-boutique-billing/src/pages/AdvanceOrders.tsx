...
export default function AdvanceOrders({ onOrderCompleted }: AdvanceOrdersProps = {}) {
  ...
  const filtered = useMemo(() => orders.filter(order => {
    ...
    const searchable = [order.deposit_id, order.customer_name, order.phone, order.product_name, order.reference_number || '', STATUS_LABELS[order.status]].join(' ').toLowerCase()
